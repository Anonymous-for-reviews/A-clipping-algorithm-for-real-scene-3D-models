...
public:
        ...
        const unsigned int e1,const unsigned int e2) const
    {
		...
		// return 4 - cut is on vertex;
        osg::Vec2 tp1((*points)[a()].x(),(*points)[a()].y()); // triangle vertices
        osg::Vec2 tp2((*points)[b()].x(),(*points)[b()].y());
        osg::Vec2 tp3((*points)[c()].x(),(*points)[c()].y());
        int ip= intersectOnPoint(tp2,tp3,p1,p2);
		if (ip == 2 && (a() == e1 || a() == e2)) { return 4; }
		if (ip == 1 && (a() == e1 || a() == e2)) { return 1; }
        ip= intersectOnPoint(tp3,tp1,p1,p2);
		if (ip == 2 && (b() == e1 || b() == e2)) { return 4; }
        if (ip==1 && (b()==e1 || b()==e2)) { return 2;}
        ip= intersectOnPoint(tp1,tp2,p1,p2);
		if (ip == 2 && (b() == e1 || b() == e2)) { return 4; }
        if (ip==1 && (c()==e1 || c()==e2)) { return 3;}
        return icut;
    }

	int whichVertex(const unsigned int e1, const unsigned int e2) const
	{
...
private:
	...
	int intersectOnPoint(const osg::Vec2 p1, const osg::Vec2 p2, const osg::Vec2 p3, const osg::Vec2 p4) const
	{
		// intersection point of p1,p2 and p3,p4
		// test from http://astronomy.swin.edu.au/~pbourke/geometry/lineline2d/
		// the intersection must be internal to the lines, not an end point.  
		// determine whether the intersection is at the vertex
		float det = ((p4.y() - p3.y())*(p2.x() - p1.x()) - (p4.x() - p3.x())*(p2.y() - p1.y()));
		if (det != 0)
		{
			// point on line is P=p1+ua.(p2-p1) and Q=p3+ub.(p4-p3)
			...
			if (ua >= 0.00 && ua < 1.00 && ub >= 0  && ub < 1.00)
			{
				return 1;
			}
		}
...
osgTest::DelaunayConstraint *getconvexhull(osg::Vec3Array *points)
{ // fits the 'rubberband' around the 2D points for uses as a delaunay constraint.
    osg::ref_ptr<osgTest::DelaunayConstraint> dcconvexhull=new osgTest::DelaunayConstraint; // make
...
bool DelaunayTriangulator::triangulate()
{
    ...
    osg::ref_ptr<osgTest::DelaunayConstraint> dcconvexhull=getconvexhull(points);
                        ...
                        if (ip2>=0 && (i>0 || prset->getMode()==osg::PrimitiveSet::LINE_LOOP))
                        {
                            ...
                            if (!edgused)
                            {
                                        ...
                                        if (tradj)
                                        {
                                            while (tradj && !tradj->usesVertex(ip2) && trisToDelete.size()<999)
                                            {
                                                ...
                                                if        (icut==1) {e1=tradj->b(); e2=tradj->c();} // icut=1 implies vertex a is not involved
													...
													int cutindex = tradj->whichVertex(e1, e2);
													if (cutindex != -1) {
														ip2 = cutindex;
														break;
													}
                                                ...
                                                if (edgeLeft.back()!=e1 && edgeRight.back()==e2 && e1!=ip2) {
													...
													if (!tradj->usesVertex(ip2))
													{
														OSG_WARN << "tradj error " << tradj->a() << " , " << tradj->b() << " , " << tradj->c() << std::endl;
														int length = trisToDelete.size();
														trisToDelete.pop_back();
														int icut;
														icut = trisToDelete[length - 2]->whichEdge(points_.get(), p1, p2, e1, e2);
														if (icut == 1) { e1 = trisToDelete[length - 2]->b(); e2 = trisToDelete[length - 2]->c(); } // icut=1 implies vertex a is not involved
														else if (icut == 2) { e1 = trisToDelete[length - 2]->c(); e2 = trisToDelete[length - 2]->a(); }
														else if (icut == 3) { e1 = trisToDelete[length - 2]->a(); e2 = trisToDelete[length - 2]->b(); }
														int cutindex = trisToDelete[length-2]->whichVertex(e1, e2);
														if (cutindex != -1) {
															ip2 = cutindex;
															break;
														}
                                ...
                                for (tri=triangles.begin(); tri!=triangles.end(); )
                                {
                                    ...
                                    deleteTri!=trisToDelete.end()&& tri != triangles.end(); )
                                    {
                                        if (&(*tri)==*deleteTri)
                                        {
                                            deleted=true;
